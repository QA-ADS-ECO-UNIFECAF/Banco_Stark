...
Foi feito trẽs testes:


#### Teste 1
teste de depósito muito pequeno e negativo (menos de um centavo):
Esperado: Erro (não permitir) ou não mudar.
Resultado: Permitiu, mas não mudou.
Caso Reprovado - Não é claro com o que acontece no sistema, deve retornar uma mensagem clara que a operação não foi realizada.
Log:

```
OK - Depósito realizado: -5e-17
SALDO tony = 1000.0
1000.000000000000000000000000000000
```

#### Teste 2
teste de depósito com dizima periódica:
Esperado: Erro (não permitir) ou não mudar.
Resultado: Permitiu e alterou, deixando a conta com dizima periodica.
Caso Reprovado - Falha na validação, não deveria permitir divisões.
Log:

```
OK - Saque realizado: 0.3333333333333333
SALDO pepper = 499.6666666666667
```

#### Teste 3
teste de transferencia para si mesmo:
Esperado: Erro (não permitir).
Resultado: Permitiu, mas não alterou.
Caso Reprovado - Falha na validação, não deveria permitir transferencias para si mesmo, exceto se for para outro banco.
Log:

```
OK - Depósito realizado: 400
OK - Transferência realizada: 1.3
SALDO banner = 400.0
```
